  ...
  async.eachSeries(
    ...
    function(service, next) {
      if (self.services[service.name]) {
        log.info('Stopping ' + service.name); // TODO: update service name in bitcore-node-mue.json
        self.services[service.name].stop(next);
      } else {
        log.info('Stopping ' + service.name + ' (not started)'); // TODO: update service name in bitcore-node-mue.json
        setImmediate(next);
      }
    },
    callback
