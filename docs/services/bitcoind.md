...
The Bitcoin Service is a Node.js interface to [MonetaryUnit Core](https://github.com/muecoin/muecore) for querying information about the MonetaryUnit Core block chain. It will manage starting and stopping `mued` or connect to several running `mued` processes. It uses a branch of a [branch of Bitcoin Core](https://github.com/bitpay/bitcoin/tree/0.12-bitcore) with additional indexes for querying information about addresses and blocks. Results are cached for performance and there are several additional API methods added for common queries.
